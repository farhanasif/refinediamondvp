...
@section('content')
	...
	<div class="col-md-9">
			...
			@if(count($sub_category_wise_product_show))
			<div class="alert alert-success" style="height: 60px">
				<h3 class="text-uppercase text-center mb-4 text-black">ALL {{ $subcategories->subcategory_name }} PRODUCT</h3>
			</div>
				<div class="row">
					@foreach($sub_category_wise_product_show as $subcategory)
					<div class="col-md-3 mb-4">
						<div class="card">
							...
							@else
								<img class="card-img-top feature-image" style="height: 150px" src="{{ asset('uploads/product/'.$subcategory->photo) }}" alt="Image">
							@endif
							<div class="card-body">
								<h4 class="card-title text-center" style="font-size: 1.1rem;">{{ str_limit($subcategory->title,17) }}</h4>
								<p class="card-text text-center">Tk <b class="text-black">{{$subcategory->current_price}}</b></p>
								<div style="text-align: center">
									<a href="{{ route('viewDetailsProduct',$subcategory->id) }}">
										<p class="btn btn-warning" class="text-center">See Details</p> 
									</a>
									<form action="{{ url('add-to-cart/'.$subcategory->id) }}" method="post">
										@csrf
										<input type="hidden" value="{{$subcategory->current_price}}" name="current_price">
		...
		<br>
		{{ $sub_category_wise_product_show->links()}}
		@else
